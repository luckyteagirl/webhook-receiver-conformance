...
| ASM-011 | Scenario plans use signed 64-bit integer logical nanoseconds relative to origin zero. A value serialized into a canonical manifest must also be an integer within the I-JSON interoperable range `-(2^53-1)` through `2^53-1`. | Ordering and persistence avoid floating-point time while matching SCHED-003; planning rejects values that are valid internally but cannot be represented by the canonical manifest profile. | high | Review before accepting a schedule range or precision outside signed integer nanoseconds or the manifest interoperability range. | accepted |
