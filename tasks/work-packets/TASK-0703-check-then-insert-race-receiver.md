# TASK-0703 — Check-then-insert race receiver
...
Implement one isolated flawed receiver whose primary intentional defect is: Use a non-atomic existence check that fails under concurrent duplicate delivery.
...
- reference_receivers/flawed/check-then-insert-race-receiver/**

**Allowed files**

- reference_receivers/flawed/check-then-insert-race-receiver/**
- tests/e2e/test_check_then_insert_race_receiver.py
...
```bash
uv run pytest -q tests/e2e/test_check_then_insert_race_receiver.py
